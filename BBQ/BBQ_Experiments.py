...
class Embedder:
    def __init__(self, name_or_path: str, device: str = "auto", dtype: str = "auto", batch_size: int = 64):
        ...
        if dtype == "auto":
            if self.is_gemma:
                # Gemma does NOT support fp16 activations
        ...
        self.st_model = None
        if self.is_gemma:
            # Gemma must go through sentence-transformers
            if not _HAS_ST:
    ...
    def encode_queries(self, texts: List[str], batch_size: int = None, normalize: bool = True) -> np.ndarray:
        """
        Gemma-specific: use encode_query if available. Otherwise fall back to encode.
        Returns np.ndarray.
        """
        bs = batch_size or self.bs
    ...
    def encode_docs(self, texts: List[str], batch_size: int = None, normalize: bool = True) -> np.ndarray:
        """
        Gemma-specific: use encode_document if available. Otherwise fall back to encode.
        """
        bs = batch_size or self.bs
...
def compute_row_sims(df: pd.DataFrame, emb: Embedder, batch_size: int = 32) -> pd.DataFrame:
    ...
    Uses Gemma's query/doc split if available.
    """
    questions = df["question"].tolist()
    contexts  = df["context"].tolist()

    # always go through the high-level helpers you added
    q_vecs = emb.encode_queries(questions, batch_size=batch_size, normalize=True)  # np.ndarray [N, D]
    c_vecs = emb.encode_docs(contexts,     batch_size=batch_size, normalize=True)  # np.ndarray [N, D]

    # both are already L2-normalized (we did it in those methods), but let’s be safe:
